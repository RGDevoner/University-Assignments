...
def tick(board):
    ...
    for i,y in board2.items():
        k=0
        p=i
        b= neighbors(i)
        for m,z in board2.items():
            if (p[0],p[1]+1)==m:
                if z==True:
                    k=k+1
            elif((p[0],p[1]-1))==m:
                ...
                if z==True:
                    k=k+1
        if (k==1 or k==0) and y==True:
            set_alive(board,(p[0],p[1]),False)
        elif k>=4 and y==True:   
            set_alive(board,(p[0],p[1]),False) 
        elif k==3 and y==False:
            set_alive(board,(p[0],p[1]),True) 
